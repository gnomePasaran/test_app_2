...
feature 'To see Top-5 one day info', %q{
  ...
  given(:users) { create_list(:user, 5, created_at: Time.current - 4.days) }
  given(:month_ago_posted_user) { create(:user) }
  given(:month_ago_post) { create(:post, user: month_ago_posted_user, created_at: Time.current - 1.month) }
  ...
  describe 'Guest try to see Top-5' do
    scenario 'the most posting users' do
      ...
      expect(page).to_not have_content month_ago_posted_user.email
    end
    ...
    scenario 'without month ago posting users' do
      expect(page).to_not have_content month_ago_posted_user.email
    end
  end

  describe 'Guest try to see Top-5 users' do
    given(:most_liked_users) { User.most_liked_users(Date.current - 1.week, Date.current) }
    ...
    scenario 'without month ago liked posts and posts itself' do
      expect(page).to_not have_content month_ago_posted_user.email
      expect(page).to_not have_content month_ago_post.text
      expect(page).to_not have_content month_ago_post.likes.count
    end
  end

  describe 'Guest try to see Top-5 users' do
    given(:most_average_users) { User.most_average_users(Date.current, Date.current - 1.week) }
    given(:average_week_ago) { User.most_average_users(Date.current - 1.month, Date.current - 2.weeks) }
    ...
    scenario 'without month ago rating posts' do
